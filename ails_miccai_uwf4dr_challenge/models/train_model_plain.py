...
# data
from ails_miccai_uwf4dr_challenge.dataset_strategy import Task1Strategy, DatasetBuilder, MiniDatasetStrategy
# models
...
# training
from ails_miccai_uwf4dr_challenge.models.trainer import DefaultMetricsEvaluationStrategy, Loaders, Metric, Trainer, \
    PersistBestModelOnEpochEndHook, OversamplingResamplingStrategy, \
    TrainingRunHardware, WandbLoggingHook, InitWandbTrainingStartHook, FinishWandbTrainingEndHook
from ails_miccai_uwf4dr_challenge.preprocess_augmentations import ResidualGaussBlur
...
def create_training_run_hardware(config, device):
    ...
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=config.lr_scheduler_cycle_epochs,
...
if __name__ == "__main__":
    ...
    config = Config(
        ...
        notes="4 Folds using shufflenet for task1. Masking is excluded. Oversampling is included and both datasets are used."
        # use this field to describe the experiment - it will show up in wandb,
    )
